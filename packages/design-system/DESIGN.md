...
| Axis | Contract |
| --- | --- |
| **Geometry** | One band, two rows, 130.6px: a 64px utility row (brand lockup · hall switcher · deliver-to · search · account · locale · cart) closed by a 1px rule, then a 63.6px category rail closed by a `--stroke-thick` `--divider-emphasis` rule. Page margin is `--space-600` on both rows; the PC brand lockup renders 48px tall at its intrinsic aspect ratio; the band caps at 1920px. |
