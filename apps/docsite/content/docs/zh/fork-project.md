...
- 确认当前登录的 GitHub 账号，但不要显示访问令牌或其他凭证。
- Fork 该仓库到我的 GitHub 账号下，不修改项目名称。
- 如果 Fork 已经存在，不要重复创建。
- 将我的 Fork 克隆到指定的新本地目录，不覆盖已有目录。
- 确认我的 Fork 是 origin，原始主项目是 upstream。
- 安装项目依赖，不删除或改写锁文件来绕过错误。
- 运行 pnpm generate。
- 只启动 Storybook，不启动 Canvas、Docsite 或其他应用。
- 在浏览器中打开 Storybook，并验证页面和真实组件正常渲染，而不只是返回 HTTP 200。
- 不修改与本次任务无关的文件，不提交、不推送、不发布。
